...
# data loading
def get_batch(split):
    # generating a batch of data of inputs x and targets y
...
class LanguageModel(nn.Module):
    def __init__(self):
        super().__init__()
        # each token directly reads off the logits for the next token from look up table
    ...
    def forward(self, idx, targets=None):
        B, T = idx.shape
        # idx and targets are both (B, T) tersor of integers
...
for iter in range(max_iters+1):
    ...
    # calculate the loss on train and val sets every once in a while 
